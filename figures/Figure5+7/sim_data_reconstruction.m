...
% Store magnitudes |a| for Figure 5 (a,b) and ground truth
...
% Store noiseless I(0,\cdot) for Figure 5(c)
...
export_fig I_noiseless -png -transparent

% Add Poisson noise
level = 1;
Ip = level*poissrnd(Ipg/level);

% Add salt-and-pepper noise

sp_num = round(0.005 * numel(Ipg));
p_num = round(sp_num / 2);
I_max = max(Ipg,[],"all");

per = randsample(numel(Ipg),sp_num,true);

Ip = reshape(Ip,[],1);
Ip(per(1:p_num)) = 0;
Ip(per((p_num+1):end)) = I_max;
Ip = reshape(Ip,size(Ipg));

fprintf('Salt and Pepper noise reconstruction, rel. noise: %.07f \n', rel_measurement_error(Ip,Ipg))

Ipn = Ip;

% Store noisy I_n(0,\cdot) for Figure 5(d)
...
export_fig I_noisy -png -transparent
...
tic
[a_x_rec, a_y_rec] = optical_flow_HS_harmonic_lsqr_pyramid(Ip,bin,2000,weights, 50, 10^-6, 1/sqrt(1.6), 0.8, 2,'lsqr');
time = toc;
...
%% Model III
tic
[a_x_rec, a_y_rec] = optical_flow_HS_harmonic_lsqr_pyramid(Ip,bin,20,weights, [100, 25], 10^-9, 1/sqrt(1.6), 0.8, 4,'l1');
time = toc;
