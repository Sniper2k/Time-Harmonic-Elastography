...
%     eps_new = 0.5 * eps_new / lambda;% / (nt*prod(d)+2);
% 
%     epsilon = min(epsilon, 0.001 * eps_new/sqrt(irls_it)); 
    sigma = sqrt(max(epsilon, res));

%     epsilon = min(epsilon, 0.001 * mean(res,'all') / sqrt(irls_it));
%     sigma = max(Rn *  Dn * lambda* epsilon, res);
    ...
    M = @(x) afun(x,flag,d,nt,2*lambda,IxIxf0,IxIxf2w,IxIyf0,IxIyf2w,IyIyf0,IyIyf2w, weights);
%     warning off
